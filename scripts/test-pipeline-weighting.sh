...
jq -c '.scenarios[]' "${SCENARIOS_FILE}" | while IFS= read -r scenario; do
  ...
  while IFS=$'\t' read -r capability minimum; do
    ...
    provided="$(capability_servings "${response_file}" "${capability}")"
    (( provided >= minimum )) ||
      fail "${scenario_id}: ${capability} provides ${provided} servings, expected at least ${minimum}"
  done < <(jq -r '.expect.minimumCapabilityServings // {} | to_entries[] | [.key, .value] | @tsv' <<< "${scenario}")
  ...
  if jq -e '.expect.noCapabilityShortfallWarning == true' <<< "${scenario}" >/dev/null; then
    jq -e 'all(.warnings[]; (startswith("Capability") | not))' "${response_file}" >/dev/null ||
      fail "${scenario_id}: returned an unexpected capability-share warning"
  fi
  ...
  capability_summary="$(
    jq -r '.request.capabilityShares // {} | keys[]' <<< "${scenario}" |
      while IFS= read -r capability; do
        printf '%s=%s ' "${capability}" "$(capability_servings "${response_file}" "${capability}")"
      done
  )"

  echo "PASS ${scenario_id}"
  echo "  meals=${meal_summary}"
  echo "  capabilityServings=${capability_summary:-none}"
